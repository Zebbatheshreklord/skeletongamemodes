...
/*---------------------------------------------------------
   Text when you chose another team by Disseminate
---------------------------------------------------------*/
...
function GM:KeyPress(p, key) 
    if ( key == IN_SPEED and ( not p.LastSwap or CurTime() - p.LastSwap > 60 ) ) then 
        ...
        p:SetTeam( TEAM_RED )
        p:Kill( )
        p:PrintMessage(HUD_PRINTTALK,"you don't want to be in the team red? Press 'SHIFT' to be one again in one minute!"); -- eg: Aliens (blue)
        p:SprintEnable ()
        p:AddFrags(1)
        else
        p:SetTeam( TEAM_BLUE )
        p:Kill( )
        p:PrintMessage(HUD_PRINTTALK,"you don't want to be in the team blue? Press 'SHIFT' to be one again in one minute!"); -- eg: Predator(red)
        p:SprintDisable ()
        p:AddFrags(1)
        end
    end
...
local mRotateTimeLeft=1800 --this is the round time in seconds (3600= 1 hr)
